...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Git Hooks Init Script — bun + biome stack
...
echo ""
print_divider "$_CLI_PURPLE" "Hook Setup (bun + biome)"
echo ""
...
check_tool "bun"   "https://bun.sh — curl -fsSL https://bun.sh/install | bash"  "bun --version"
...
# biome ships as a devDependency in package.json; verify via bunx
if bunx --bun biome --version >/dev/null 2>&1; then
  log_success "biome ${_CLI_GRAY}($(bunx --bun biome --version 2>/dev/null | head -n 1))${_CLI_NC}"
else
  log_error "biome ${_CLI_RED}not available via bunx${_CLI_NC} — add @biomejs/biome to devDependencies and run \`bun install\`"
  MISSING_TOOLS+=("biome|bun add -D @biomejs/biome")
fi

# gitleaks is OPTIONAL — only required if .gitleaks.toml exists
...
echo -e "  ${_CLI_GRAY}•${_CLI_NC} pre-push    — gitleaks + bun run check + tsc --noEmit + bun test (if defined)"
echo ""
