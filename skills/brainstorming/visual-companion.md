...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.agents/brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to add `.agents/brainstorm/` to `.gitignore` if it's not already there.

**Launching the server by platform:**

**pi and other environments that preserve background processes:**
```bash
